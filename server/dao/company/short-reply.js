...
class ShortReplyDao {
    ...
    getShortReplyList (page , pageSize , type , commentId , replyId) {
        let sql = `select * from \`tb_company_comment_reply-list\` where 1`;
        (type && type != 'A') && (sql += ` and CCR_IsAdminCheck = '${type}'`);
        commentId && (sql += ` and CCR_CommentID = '${commentId}'`);
        replyId && (sql += ` and CCR_ID = '${replyId}'`);
        sql += ` order by CCR_AddTime desc limit ${(page - 1) * pageSize} , ${pageSize}`;
    ...
    getReplyContent (replyId) {
        let sql = `select * from \`tb_company_comment_reply-content\` where CCR_CommentReplyID='${replyId}'`;
        return this._select(companyDB , sql);
    }
    // 获取公司长评列表
    ...
    getOneCommentInfo (commentId) {
        let sql = `select * from \`tb_company_comment-content\` where CC_CommentID='${commentId}'`;
        return this._select(companyDB , sql);
    }
    // 通过公司id获取公司信息
...
module.exports = new ShortReplyDao();